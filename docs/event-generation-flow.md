...
![Simulator 内部事件影响因素](./assets/simulator-event-generation-logic.png)

## 用户、家电和自动化如何形成因果链

事件生成器不会只让设备随机变化。大多数可控家电状态变化都会带有一个可解释原因：来自场景计划、用户控制、家庭成员活动、自动化规则或外部异常。完整原始事件会保留这些原因；device-events 只保留设备字段变化，供 memory 做隐私更窄的观察。

### 默认用户行为逻辑

默认家庭包含 `adult_1`、`adult_2`、`child_1` 和 `pet_1`。用户行为有两层来源：场景步骤提供一天中的主线安排，动态策略在每分钟根据需求、时间、资源和当前活动补充行为。

| 用户 | 主线行为 | 会带动的设备和事件 |
| --- | --- | --- |
| `adult_1` | 通勤成人。工作日早起、洗漱、早餐、离家通勤，晚上回家、晚饭、看电视、睡觉 | 早餐打开厨房/餐厅灯和冰箱；离家和回家影响门锁；回家触发客厅到家场景，打开客厅灯并调整窗帘；也常被选为离设备最近的操作者或家务响应者 |
| `adult_2` | 远程工作成人。早上进入厨房，白天在书房远程工作，晚上通常做饭 | 远程工作会更新书房 CO2 和路由器负载；做饭会打开炉灶，并通过规则带动油烟机和厨房灯；可参与包裹、访客、维修、家务等社会协调 |
| `child_1` | 学生。工作日离家上学，下午回家做作业，晚上参与晚饭、看电视和睡觉 | 做作业会关闭电视、调暗客厅灯，并标记儿童睡眠传感器离床；如果作业压力高且仍在娱乐，家长提醒会把活动转为 homework |
| `pet_1` | 宠物。白天更频繁、睡眠模式下更低频地在客厅、厨房、餐厅、花园和主卧之间移动 | 影响运动传感器和摄像头；如果进入花园且喷淋正在打开，会触发喷淋暂停 |

动态策略不是全量自由规划。它只在明确候选场景下介入，例如白天仍在睡觉、早晨或午间饥饿、资源库存支持做饭或点餐、任务压力要求作业。选择活动时会综合 persona 偏好、需求值、资源可用性、承诺压力、家庭请求和资源冲突。

### 可控家电的使用条件

设备控制分为两类。外部控制入口会先校验设备和命令，再选择一个在家的操作者，生成“走到设备旁、控制设备、返回原位置”的人员移动事件；如果没有本地操作者，仍可记录为外部或远程控制。日常模拟中的设备使用则通常由人员活动或规则触发。

| 设备 | 生成逻辑 |
| --- | --- |
| 灯 | 醒着的人进入有灯房间时会打开对应灯；由 habit 打开的灯会在人离开后关闭；睡眠和离家模式会关闭公共灯 |
| 空调 | 客厅、主卧和儿童房支持舒适控制。房间有人且温度过热时制冷，过冷时制热；房间空了且空调是规则控制的会关闭 |
| 冰箱 | 早餐、做饭或直接命令会改变门状态。门保持打开时，每分钟累计打开时长、提升功耗，并影响厨房温度；规则会创建冰箱门未关告警，长开后升级 |
| 炉灶 | 早餐、晚饭或周末 brunch 会开启炉灶。厨房有人且功率较高时，自动开启油烟机和厨房灯；厨房无人且高功率时，安全规则关闭炉灶 |
| 油烟机 | 做饭规则打开，做饭结束或安静模式关闭 |
| 洗碗机 | 脏盘子达到阈值、饭后、非睡眠/离家、并且有醒着的人在家时，选择一个附近家庭成员去厨房装载并启动；倒计时结束后进入等待取出状态并创建提醒 |
| 洗衣机 | 脏衣物达到阈值、白天窗口、非睡眠/离家、并且有醒着的人在家时，选择家庭成员去卫生间装载并启动；倒计时结束后进入等待取出状态并创建提醒 |
| 扫地机器人 | 白天、非睡眠/离家、有人醒着在家、客厅无人且机器人停靠时，按概率启动清扫；运行中可能报告卡住，需要协助；睡眠模式会让它回桩 |
| 电视 | 晚间家庭娱乐或雨天室内活动打开；孩子做作业、睡眠或离家模式会关闭 |
| 路由器 | 远程工作会提高网络负载上下文；网络异常会告警；重启命令会进入 restarting、reconnecting、recovered 生命周期 |
| 门锁 | 离家、回家、异常注入或直接命令会改变门锁。全员离家且门已锁时触发离家模式；门未关异常会创建入口告警 |
| 水阀和漏水传感器 | 漏水传感器激活且水阀仍开时，安全规则关闭水阀并进入告警流程 |
| 喷淋 | 夏季日程或园艺活动会打开喷淋；宠物进入花园时会自动暂停 |
| 窗帘 | 通勤者回家后会降低客厅窗帘，形成到家隐私场景；也支持直接位置控制 |

### 自动化规则的前提

自动化规则主要读取当前快照，而不是回看完整历史。它们的前提通常由 home mode、房间占用、人员活动、设备字段、库存资源、时间窗口和告警状态组成。

| 规则类型 | 前提条件 | 结果 |
| --- | --- | --- |
| 睡眠模式 | `homeState.mode` 是 sleeping | 关闭公共灯、电视和油烟机，让扫地机器人回桩，暂停非人工启动的家务电器 |
| 离家模式 | 所有人类成员都 away，且门锁已锁 | 设置 home mode 为 away，布防安全模式，并关闭非必要公共设备 |
| 做饭通风 | 厨房有人，炉灶功率超过阈值 | 打开油烟机和厨房灯，记录 cooking_ventilation 自动化 |
| 炉灶安全 | 炉灶高功率运行且厨房无人 | 关闭炉灶，记录 unattended safety 自动化 |
| 漏水响应 | 漏水传感器为 true 且水阀还开着 | 关闭主水阀，创建高优先级漏水响应 |
| 冰箱门告警 | 冰箱门打开且处在 opened、still_open 或 alert 生命周期 | 创建关闭冰箱门提醒，后续长开会升级能耗和温漂影响 |
| 网络异常 | 路由器离线且来源是网络异常注入 | 创建网络离线告警，并建议重启路由器 |
| 门口异常 | 门锁未锁、门口摄像头有运动，且来源是门未关异常注入 | 创建入口安全告警 |
| 作业提醒 | 孩子在娱乐活动中，且作业压力较高，且有可用家长 | 生成家长提醒对话，并把孩子活动转为 homework |
| 晚饭邀请 | 晚饭时间窗口内，有人在厨房做饭，且有可用家庭成员 | 生成家庭邀请对话，移动可用成员到餐厅，并设置餐厅灯和厨房设备安全状态 |
| 包裹、访客、维修、药品补充 | 外部信号或库存状态满足条件，且有可用成人或家庭成员 | 生成外部交互、人员移动、设备状态清理和自动化解释 |
| 资源冲突 | 多人同时请求同一共享资源，例如书房或洗手池 | 优先级最高的人继续使用，其它人进入等待状态 |

这些规则的目标是让设备变化有现实前提：例如洗衣机和洗碗机需要有人装载，空调需要房间有人，扫地机器人不能在睡眠或离家模式启动，孩子作业需要家长提醒而不是设备匿名改变。
