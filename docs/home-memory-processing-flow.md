...
![家庭 Memory 处理流程信息图](./assets/home-memory-processing-flow-infographic.png)

## 一句话总览

家庭 memory 不读取模拟内部的人员真值，也不读取场景控制原因。它只消费设备值事件：哪个家庭、哪个房间、哪个设备、哪个字段，在什么时间变成了什么值。系统先把这些低层事实归一成 capability，再聚合成房间、设备、字段和 episode，从中提取 semantic signal，最后生成可解释的家庭画像假设。
...
```mermaid
flowchart LR
  Event["设备值事件<br/>deviceId / field / value / simTime"]
  Run["run 校验<br/>run 变化时重置 memory"]
  Bucket["时间归类<br/>morning / daytime / evening / night"]
  Capability["Capability<br/>presence / climate / water / system"]
  Evidence["MemoryEvidence<br/>分类、强度、能力、画像权重"]
  Field["Field memory<br/>当前值、历史、统计、近期证据"]
  Device["Device memory<br/>字段集合、事件计数、时间段分布"]
  Room["Room memory<br/>活跃设备、活跃字段、房间权重"]
  Episode["Episode<br/>连续活动片段"]
  Summary["Daily / weekly summary<br/>长窗口房间、设备、时间段"]
  Semantic["Semantic signal<br/>生活语义解释"]
  Home["HomeMemory<br/>近期证据、画像权重、语义计数"]
  Profile["Profile hypothesis<br/>家庭习惯、房间功能、人数估计"]
  Views["3D 图谱 / 查询接口<br/>展示和外部 agent 查询"]

  Event --> Run --> Bucket --> Capability --> Evidence
  Evidence --> Field --> Device --> Room --> Home
  Evidence --> Episode --> Home
  Evidence --> Summary --> Home
  Evidence --> Semantic --> Home
  Home --> Profile --> Views
  Semantic --> Profile
  Episode --> Profile
  Summary --> Profile
...
4. 根据 deviceType、field 和 value 先归一出 capability，例如 presence_detection、climate_control、water_flow、environment_temperature 或 system_health。
5. 根据 capability、deviceType、field 和 value 判断证据类别、证据强度和基础画像权重。
6. 判断这次值变化是否有意义。重复值仍会保存为事实，但画像权重为零。
7. 生成 MemoryEvidence，作为之后所有解释的底层证据。
8. 更新 field memory：当前值、上一值、变化次数、遥测次数、数值范围、布尔统计和近期事件。
9. 更新 device memory：最新字段值、字段列表、事件计数、时间段分布和证据权重。
10. 更新 room memory：活跃设备、活跃字段、事件计数、时间段分布和证据权重。
11. 更新低层 episode：把连续的低层活动压缩成占用、接触活动、设备使用或家电使用片段。
12. 更新每日和每周摘要：记录长窗口内活跃房间、设备、字段、时间段和有意义房间。
13. 生成 semantic signal：把低层字段变化解释成生活语义。
14. 更新高层 activity episode：把 semantic signal 序列解释成 return_home、meal_preparation、bedtime 或 climate_response。
15. 更新根 HomeMemory：总事件数、近期证据、画像证据权重、semantic 计数和摘要计数。
...
| MemoryEvidence | 单条设备值事件加 capability、分类、强度、权重、变化判断和解释 | 每条设备值事件转换得到 | 作为所有画像结论的可追溯证据 |
| Semantic signal | 设备字段变化对应的生活语义 | meaningful evidence 加规则识别 | 连接低层事实和高层 hypothesis |
| Low-level episode | 连续设备活动片段，如 occupancy、contact activity、device usage、appliance usage | 字段状态的开关、功率或占用信号 | 减少高频事件噪声，辅助 presence 和 household size |
| Activity episode | 高层家庭流程，如 return_home、meal_preparation、bedtime、climate_response | semantic signal 的时间邻近关系 | 表达“事件进入后形成了什么生活流程” |
...
每条设备值事件都会先归一出 capability，再变成 MemoryEvidence。capability 是设备能力语义，不是画像结论；它让新设备字段先映射到通用能力，再由下游 semantic 和 hypothesis 使用。

| Capability | 典型字段或设备 | 下游含义 |
| --- | --- | --- |
| presence_detection | motion、occupancy、peopleCount、presence 传感器 | presence_signal、人类活动证据 |
| climate_control | air conditioner、thermostat、hvac、mode=cooling/heating | climate_signal、气候响应 episode |
| water_flow | flow、valve、水表 | water_signal、卫生间/厨房活动 |
| sleep_context | sleep sensor、inBed、asleep | sleep_signal、睡眠区和人数估计 |
| environment_temperature / humidity / air_quality | temperature、humidity、CO2、PM2.5 | environment_signal，作为弱背景 |
| system_health | battery、online、firmware、signal | system_signal，不参与画像 |

MemoryEvidence 的分类决定这条事件对画像有多强。
...
| system_status | 电池、在线状态、固件、信号强度、confidence、remainingMin、lifecyclePhase | 设备健康、质量或生命周期元数据 | 保存为事实，但不参与画像推断 |
...
| 同一 source event 带 confidence | confidence 作为 0 到 1 的质量系数缩放其它测量的画像权重；confidence 字段本身权重为零 |

这个设计让高频传感器不会因为重复上报而过度影响画像，也避免把质量分数、倒计时和生命周期标签误当作独立行为。
...
| climate_signal | 空调、暖气、恒温器等气候控制设备 active | 主动调节室内环境 | climate_response、设备贡献 |
...
Episode 分两层：低层 episode 把连续的设备事件压缩成行为片段，高层 activity episode 把多个 semantic signal 串成生活流程。
...
低层 episode 会保存开始时间、更新时间、结束时间、持续时长、房间、设备、字段、证据 ID、最新值、峰值和累计画像权重。它主要用于 presence、routine 和 household size。

高层 activity episode 目前支持：

| Activity episode | 触发方式 | 含义 |
| --- | --- | --- |
| return_home | access_signal 后 30 分钟内出现其它房间的行为 semantic | 可能的回家后第一跳流程 |
| meal_preparation | cooking_signal 附近出现厨房 presence、water、lighting 或 cooking signal | 可能的备餐或厨房活动 |
| bedtime | sleep_signal 出现 | 睡眠或上床流程 |
| climate_response | environment_signal 后 30 分钟内同房间出现 climate_signal | 环境变化后的主动气候调节 |
...
| household_size | 直接人数、重叠 occupancy episode、睡眠区、routine cluster、长期摘要、弱环境占比 | 估计 1 到 5 人概率分布、下界和置信度 | 家庭常住人数的概率画像 |
...
| 特征 | 数据来源 | 作用 |
| --- | --- | --- |
| 直接人数 | peopleCount、personCount、occupancyCount 等数值字段的历史最大值 | 可以提供硬人数下界 |
| 重叠 occupancy | 从完整低层 occupancy episode 中寻找时间上真正重叠的房间 | 作为多人活动的软证据；运动传感器滞留、人员移动或宠物活动不能单独提高硬下界 |
...
多房间 occupancy 必须在 episode 时间区间上真正重叠。系统仍会把最强重叠时刻归到 10 分钟刻度，方便展示和 trace，但这个刻度不再用于把相邻事件推成“同时在场”，也不再直接设置人数下界。
